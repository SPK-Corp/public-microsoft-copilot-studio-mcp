# GitHub Copilot 에이전트 모드에서 MCP 서버 사용하기
...
![Visual Studio Code에서 MCP 서버 설정 안내](img/3-4-1.png)
![에이전트 세션별 도구 선택](img/3-4-2.png)
![MCP 개발 중 오류를 쉽게 디버깅](img/3-4-3.png)
...
1. *mcp.json*에서 항목을 찾아 "재생" 아이콘을 확인하세요:
  ...
  ![Visual Studio Code에서 서버 시작](img/3-4-4.png)  
...
1. "재생" 아이콘을 클릭하면 GitHub Copilot 채팅의 도구 아이콘에 사용 가능한 도구 수가 증가하는 것을 볼 수 있습니다. 해당 도구 아이콘을 클릭하면 등록된 도구 목록이 표시됩니다. 각 도구를 체크하거나 체크 해제하여 GitHub Copilot이 이를 컨텍스트로 사용할지 여부를 결정할 수 있습니다:
  ...
  ![Visual Studio Code에서 도구 시작](img/3-4-5.png)

1. 도구를 실행하려면 도구 설명과 일치하는 프롬프트를 입력하세요. 예를 들어 "22에 1을 더해줘"와 같은 프롬프트를 입력합니다:

  ![GitHub Copilot에서 도구 실행](img/3-4-6.png)
...
- 다음: [MCP 서버와 stdio 전송 방식](<../3-5. Stdio Server/3-5.md>)