...
> **중요 업데이트**: MCP 사양 2025-06-18 기준으로, 독립형 SSE(Server-Sent Events) 전송 방식은 **더 이상 사용되지 않으며** "Streamable HTTP" 전송 방식으로 대체되었습니다. 현재 MCP 사양은 두 가지 주요 전송 메커니즘을 정의합니다:
...
- **다음**: [MCP의 HTTP 스트리밍(Streamable HTTP)](<../3-6. Http Streaming/3-6.md>) - 원격 서버를 위한 다른 지원 전송 메커니즘 배우기
- **운영**: 배포 전략(작성중) - 운영 환경에서 서버 배포하기
