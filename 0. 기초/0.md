...
**참고:** MCP는 개방형 표준으로 홍보되지만, IEEE, IETF, W3C, ISO 또는 기타 표준 기구를 통해 MCP를 표준화할 계획은 없습니다.
...
graph TD
    ...
    style H fill:#eeeeee,stroke:#333,stroke-width:2px,color:#000
    style Client fill:#d5e8f9,stroke:#333,stroke-width:2px,color:#000
    style G fill:#fffbe6,stroke:#333,stroke-width:1px,color:#000
    style I fill:#fffbe6,stroke:#333,stroke-width:1px,color:#000
    style J fill:#fffbe6,stroke:#333,stroke-width:1px,color:#000
    style K fill:#fffbe6,stroke:#333,stroke-width:1px,color:#000
    style T1 fill:#c2f0c2,stroke:#333,stroke-width:1px,color:#000
    style T2 fill:#c2f0c2,stroke:#333,stroke-width:1px,color:#000
    style T3 fill:#c2f0c2,stroke:#333,stroke-width:1px,color:#000
    style T4 fill:#c2f0c2,stroke:#333,stroke-width:1px,color:#000
```

이 다이어그램은 MCP 클라이언트와 MCP 호스트, AI 모델, 여러 MCP 서버 사이에서 요청이 어떻게 흐르고, MCP 호스트 내부의 구성 요소(도구 레지스트리, 인증, 요청 처리기, 응답 포맷터)가 각각 어떤 역할을 하는지를 한눈에 보여줍니다.
...
graph TD
    ...
    style LLM color:#fff
    style ServerA color:#fff
    style ServerB color:#fff
    style KnowledgeA color:#fff
    style ToolsA color:#fff
    style KnowledgeB color:#fff
    style ToolsB color:#fff
```
...
```mermaid
---
title: 클라이언트-서버 LLM 통합을 활용한 심화 MCP 시나리오
description: A sequence diagram showing the detailed interaction flow between user, client application, client LLM, multiple MCP servers, and server LLM, illustrating tool discovery, user interaction, direct tool calling, and feature negotiation phases
---
%%{init: {'theme':'base', 'themeVariables': {'actorTextColor':'#000', 'labelTextColor':'#000', 'noteTextColor':'#000'}}}%%
sequenceDiagram
    autonumber
    actor User as 사용자
    ...
    rect rgb(220, 240, 255)
        ...
        Note right of ClientApp: 통합된 도구 카탈로그를<br/>로컬에 저장
    end
...
위 시퀀스 다이어그램은 클라이언트 측 LLM이 MCP 서버들과 상호작용하는 전체 흐름을 네 단계로 나누어 보여줍니다:

- **도구 디스커버리 단계**: Client App이 MCP Server 1, MCP Server 2에서 사용 가능한 도구와 리소스를 조회하고, 통합된 도구 카탈로그를 로컬에 구성합니다.
- **사용자 상호작용 단계**: 사용자가 자연어 프롬프트를 입력하면 Client App이 프롬프트와 도구 카탈로그를 Client LLM에 전달하고, LLM이 어떤 도구를 사용할지 결정합니다.
- **시나리오 A – Direct Tool Calling**: Client LLM이 특정 도구 실행을 요청하면 Client App이 MCP Server 1에 직접 도구 호출을 보내고, 결과를 받아 처리한 뒤 최종 응답을 생성해 사용자에게 반환합니다.
- **시나리오 B – Feature Negotiation**: Client LLM이 필요한 기능을 식별하면 Client App이 MCP Server 2와 기능과 능력을 협상하고, Server LLM이 추가 컨텍스트를 제공한 뒤 협상된 도구를 호출하여 얻은 결과를 바탕으로 최종 응답을 생성합니다.
